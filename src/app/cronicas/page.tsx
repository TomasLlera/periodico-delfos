...
/**
 * El listado de crónicas. Segunda mitad del Step 9 del Build Order.
 *
...
export default async function Cronicas({ searchParams }: Params) {
  const pagina = paginaPedida((await searchParams).pagina)

  // Sin proyecto de Supabase el listado se dibuja vacío en lugar de reventar.
  const { notas, total } = haySupabase()
    ? await getNotasPorCategoria('cronica', pagina, POR_PAGINA)
    : { notas: [], total: 0 }
